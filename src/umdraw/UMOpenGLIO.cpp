...
namespace
{
		...
		UMNodePtr src_node)
	{
		...
		std::vector<UMVec3d> points;
		if (UMSoftwareIO::convert_node_to_octahedron(octahedron, points, src_node))
		{
			std::vector<UMVec3f> pointsf(points.size());
			for (size_t i = 0; i < points.size(); ++i)
			{
				pointsf[i] = to_float(points[i]);
			}
			...
			glBufferData(GL_ARRAY_BUFFER,
				sizeof (UMVec3f) * pointsf.size(),
				reinterpret_cast<const GLvoid*>( &(*pointsf.begin()) ), 
				GL_STATIC_DRAW );
	...
	UMNodePtr src)
{
	...
	if (load_node(node, src))
	{
		...
		material->set_diffuse(src->node_color());
		UMOpenGLMaterialPtr gl_material = UMOpenGLIO::convert_material_to_gl_material(material);
	...
	UMNodePtr src)
{
	...
	{
		UMVec4f color = to_float(src->node_color());
		deform_node->mutable_material_list().at(0)->set_diffuse(color);
		return true;
	}
	return false;
}
