...
private:
	...
	UMOpenGLShaderManagerPtr temporary_line_shader_manager_;
	...
	UMOpenGLLineList gl_temporary_line_list_;
...
bool UMOpenGLScene::Impl::init(UMScenePtr scene)
{
	...
	temporary_line_shader_manager_ = std::make_shared<UMOpenGLShaderManager>();
	temporary_line_shader_manager_->init(UMOpenGLShaderManager::eModel);
...
bool UMOpenGLScene::Impl::update()
{
	...
	// update temporary lines
	if (scene_->is_visible(UMScene::eTemporaryLine))
	{
		update_temporary_lines();
	}
	// update models
	if (scene_->is_visible(UMScene::eMesh))
	{
 ...
 * update temporary lines
 */
void UMOpenGLScene::Impl::update_temporary_lines()
{
	bool is_temporary_changed = false;
	if (temporary_line_list_.size() != scene_->temporary_line_list().size())
	{
		is_temporary_changed = true;
	}
	else
	{
		UMLineList::const_iterator it = scene_->temporary_line_list().begin();
		for (; it != scene_->temporary_line_list().end(); ++it)
		{
			UMLinePtr line = (*it);
			if (line->is_changed())
			{
	...
	if (is_temporary_changed)
	{
		...
		for (; it != scene_->temporary_line_list().end(); ++it)
		{
			...
			if (gl_line) {
				gl_temporary_line_list_.push_back(gl_line);
			}
		}
		temporary_line_list_ = scene_->temporary_line_list();
	}
}
	...
	{
		{
			...
			if (scene_->is_visible(UMScene::eTemporaryLine))
			{
				UMOpenGLLineList::iterator it = gl_temporary_line_list_.begin();
				for (; it != gl_temporary_line_list_.end(); ++it)
				{
					draw_parameter_->set_shader_manager(temporary_line_shader_manager_);
					(*it)->draw(draw_parameter_);
				}
			}
			// draw models
...
bool UMOpenGLScene::Impl::load(UMScenePtr scene)
{
	...
	{
		UMLineList::const_iterator it = scene->temporary_line_list().begin();
		for (; it != scene->temporary_line_list().end(); ++it)
		{
			UMLinePtr line = *it;
			UMOpenGLLinePtr gl_line = UMOpenGLIO::convert_line_to_gl_line(line);
			if (gl_line) {
