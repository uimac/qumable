...
static UMMappingGUIPtr create_initial_gui_scene(
	umdraw::UMScenePtr scene,
	int width, 
	int height)
{
	UMMappingGUIPtr gui_scene = UMMappingGUI::create();
	gui_scene->set_umdraw_scene(scene);
	gui_scene->init(width, height);
	return gui_scene;
}
...
int UMWindow::main(int argc, char** argv)
{
	...
	// create main window
	GLFWwindow* window = glfwCreateWindow(width_, height_, "qumable", NULL, NULL);
	if (!window) {
		glfwTerminate();
		exit( EXIT_FAILURE );
	}
	...
	{
		if (UMMappingGUIPtr gui_scene = create_initial_gui_scene(scene, width_, height_))
		{
