...
bool UMRT::add_pick_node_list(umdraw::UMScenePtr scene)
{
	if (!scene) return false;
	if (scene->node_list().empty()) return false;
	scene_access_->add_pick_node_list(scene);
	return true;
}
 ...
 * pick
 */
umdraw::UMNodePtr UMRT::pick(double x, double y)
{
	if (!scene_access_) return umdraw::UMNodePtr();
	return scene_access_->pick(x, y);
}
