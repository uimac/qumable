...
namespace svg
{
    ...
    void parser::parse_rect(const char** attr)
    {
		...
		if (rx == 0.0 && ry != 0) { rx = ry; }
		if (rx != 0.0 && ry == 0) { ry = rx; }
        ...
        if(w != 0.0 && h != 0.0)
        {
			...
			m_path.curve4(x + w - rx / 2.0, y, x + w, y + ry / 2.0, x + w, y + ry);
            m_path.line_to(x + w, y + h - ry);
			m_path.curve4(x + w, y + h - ry / 2.0, x + w - rx / 2.0, y + h, x + w - rx, y + h);
            m_path.line_to(x + rx,     y + h);
			m_path.curve4(x + rx / 2.0, y + h, x, y + h - ry / 2.0, x, y + h - ry);
			m_path.line_to(x, y + ry);
			m_path.curve4(x, y + ry / 2.0, x + rx / 2.0, y, x + rx, y);
			m_path.line_to(x + rx, y);
