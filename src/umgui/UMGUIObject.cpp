...
bool UMGUIObject::intersect(double x, double y) const
{
	...
	double initwidth = 1.0 / 1280.0;
	double initheight = 1.0 / 720.0;
	double minx = box_.minimum().x * initwidth;
	double maxx = box_.maximum().x * initwidth;
	double miny = box_.minimum().y * initheight;
	double maxy = box_.maximum().y * initheight;
